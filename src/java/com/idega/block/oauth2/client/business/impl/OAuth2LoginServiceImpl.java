...
public class OAuth2LoginServiceImpl extends DefaultSpringBean implements
	...
	protected boolean login(User user) {
		...
		try {
			...
			com.idega.user.data.bean.User userEntity = userDAO.getUser(Integer.valueOf(user.getId()));

			LoginBusinessBean.getDefaultLoginBusinessBean().logInUser(iwc, userEntity);
		} catch (Exception e) {
