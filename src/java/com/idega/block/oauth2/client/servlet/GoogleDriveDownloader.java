...
public class GoogleDriveDownloader extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		Credential credential = getGoogleAuthorizationService().getCredential(request.getParameter(PARAMETER_ACCOUNT));

		if (credential != null) {
			String appName = request.getParameter("appName");
			Drive drive = getGoogleDriveService().getDrive(URLDecoder.decode(appName, CoreConstants.ENCODING_UTF8), credential);
			if (drive != null) {
				...
				if (service != null) {
					...
					if (StringUtil.isEmpty(downloadUrl)) {
						mimeType = MimeTypeUtil.MIME_TYPE_PDF_1;
